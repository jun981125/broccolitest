...
@Controller
public class AnmtUpdateController {
	@Autowired
	private AnmtDao dao;
	...
	public String edit(@RequestParam("num") String num, @RequestParam("page") String page, Model model) {
		...
		return "noticeboard/anmtupdate";
	}

	@PostMapping("Aupdate")
	public String editProcess(AnmtBean bean, @RequestParam("page") String page, BindingResult result,
			...
			if (true) { // 아이디비교
				...
				try {
					...
					if (b) {
						...
						return "redirect:Adetail?num=" + bean.getNum() + "&page=" + page;
					} else {
						return "redirect:err";
					}
				} catch (Exception e) {
